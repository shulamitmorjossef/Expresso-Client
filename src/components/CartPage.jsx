...
export default function CartPage() {
  ...
  const [cartItems, setCartItems] = useState([]);
  const [subtotal, setSubtotal] = useState(0);
  const [originalSubtotal, setOriginalSubtotal] = useState(0); // חדש
  const [couponCode, setCouponCode] = useState(""); // חדש
  const [couponApplied, setCouponApplied] = useState(false); // חדש
  const [agreed, setAgreed] = useState(false);
  const [showWarning, setShowWarning] = useState(false);
  ...
  const calculateSubtotal = (items) => {
    ...
    setSubtotal(fixed);
    setOriginalSubtotal(fixed); // שמירה של הסכום המקורי
  };
