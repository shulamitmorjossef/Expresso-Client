...
export default function FrotherCatalog() {
  ...
  useEffect(() => {
    axios.get(`${baseUrl}/get-all-milk-frothers`)
      ...
      .catch(err => {
        ...
        setModalData({
          title: 'Error',
          message: 'Failed to load milk frothers.',
          actionText: 'OK',
  ...
  const handleDelete = (id) => {
    setModalData({
      ...
      message: 'Are you sure you want to delete this frother?',
      actionText: 'Yes',
      cancelText: 'No',
      onAction: async () => {
        setModalData(null);
        try {
          await axios.delete(`${baseUrl}/delete-milk-frother/${id}`);
          setFrothers(frothers.filter(f => f.id !== id));
          setModalData({
            title: 'Deleted',
            message: 'Frother deleted successfully.',
            actionText: 'OK',
            onAction: () => setModalData(null)
          });
        } catch (err) {
          ...
          setModalData({
            title: 'Error',
            message: 'Failed to delete frother.',
            actionText: 'OK',
            onAction: () => setModalData(null)
          });
        }
