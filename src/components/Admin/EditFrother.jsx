...
export default function EditFrother() {
  ...
  useEffect(() => {
    axios.get(`${baseUrl}/get-milk-frother/${id}`)
      ...
      .catch(err => {
        ...
        setModalData({
          title: 'Error',
          message: 'Failed to load frother data. Please try again.',
          actionText: 'OK',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModalData({
        title: 'Success',
        message: 'Frother updated successfully.',
        actionText: 'OK',
        onAction: () => {
          setModalData(null);
          navigate('/FrotherCatalog');
      ...
      setModalData({
        title: 'Error',
        message: err.response?.data?.message || 'Failed to update frother.',
        actionText: 'OK',
        onAction: () => setModalData(null)
      });
    } finally {
    ...
    <div className="form-background">
      <form className="edit-milk-frother-form" onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        {existingImage && (
          <>
            <label>Current Image:</label><br />
            <img src={existingImage} alt="frother" width="100" style={{ marginBottom: '10px' }} />
          </>
        )}
