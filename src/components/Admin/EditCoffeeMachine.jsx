...
//       {form.image_path && <img src={`${baseUrl}${form.image_path}`} alt="machine" width="100" />}
...
//   useEffect(() => {
//     axios.get(`http://localhost:3000/get-coffee-machine/${id}`)
//       .then(res => {
//         // Convert numeric values to strings for form inputs
//         const formattedData = {
//           ...res.data,
...
<<<<<<< HEAD
  ...
  const handleProductChange = async (e) => {
    ...
    if (productId) {
      try {
        const response = await axios.get(`${baseUrl}/get-coffee-machine/${productId}`);
...
<<<<<<< HEAD
    ...
    try {
      const response = await axios.put(`${baseUrl}/update-coffee-machine/${selectedProductId}`, data, {
...
<<<<<<< HEAD
        ...
        <input type="text" id="capacity" value={formData.capacity} onChange={handleChange} />
